...
class TestIcebergS3SignParams {
  ...
  static final String tableLocation = warehouseLocation + "ns/table1_cafebabe";
  static final String oldBaseLocation = "s3://old-bucket/ns/table1";
  static final String metadataLocation = tableLocation + "/metadata/metadata.json";

  static final LocationProvider defaultLocationProvider =
      LocationProviders.locationsFor(tableLocation, Map.of());

  static final LocationProvider objectStoreLocationProvider =
      LocationProviders.locationsFor(
          tableLocation, Map.of(TableProperties.OBJECT_STORE_ENABLED, "true"));

  // S3 URIs generated by Iceberg's location providers
  static final String dataFileS3Uri = defaultLocationProvider.newDataLocation("file1.parquet");
  static final String dataFileS3UriObjectStorage =
      objectStoreLocationProvider.newDataLocation("file1.parquet");

  // HTTP URIs for signing requests (what clients send to the signing endpoint)
  // These are the HTTP equivalents of the S3 URIs above
  static final String s3BucketHttpBase = "https://bucket.s3.amazonaws.com/warehouse/";
  static final String dataFileUri =
      dataFileS3Uri.replace("s3://bucket/warehouse/", s3BucketHttpBase);
  static final String dataFileUriObjectStorage =
      dataFileS3UriObjectStorage.replace("s3://bucket/warehouse/", s3BucketHttpBase);

  static final String oldDataFileUri =
      "https://old-bucket.s3.amazonaws.com/ns/table1/data/file1.parquet";

  static final String metadataJsonUri =
      s3BucketHttpBase + "ns/table1_cafebabe/metadata/metadata.json";
  ...
  static Stream<Arguments> addUris(String method) {
    return Stream.of(arguments(method, dataFileUri), arguments(method, dataFileUriObjectStorage));
  }

  @Test
  void checkLocation() {
    String requestedDefault = defaultLocationProvider.newDataLocation("test-file.parquet");
    String requestedObjectStorage =
        objectStoreLocationProvider.newDataLocation("test-file.parquet");

    // Both default and object storage layout URIs should be within the table location
    soft.assertThat(IcebergS3SignParams.checkLocation(requestedDefault, tableLocation)).isTrue();
    soft.assertThat(IcebergS3SignParams.checkLocation(requestedDefault, tableLocation + "/"))
        .isTrue();

    soft.assertThat(IcebergS3SignParams.checkLocation(requestedObjectStorage, tableLocation))
        .isTrue();
    soft.assertThat(IcebergS3SignParams.checkLocation(requestedObjectStorage, tableLocation + "/"))
        .isTrue();

    // Verify that URIs outside the table location are rejected
    soft.assertThat(IcebergS3SignParams.checkLocation(requestedDefault, "s3://other-bucket/path"))
        .isFalse();
    soft.assertThat(
            IcebergS3SignParams.checkLocation(requestedObjectStorage, "s3://other-bucket/path"))
        .isFalse();

    // Verify that URIs from a different table are rejected
    String otherTableLocation = warehouseLocation + "ns/other_table";
    soft.assertThat(IcebergS3SignParams.checkLocation(requestedDefault, otherTableLocation))
        .isFalse();
    soft.assertThat(IcebergS3SignParams.checkLocation(requestedObjectStorage, otherTableLocation))
        .isFalse();
  }
  ...
  void verifyAndSignFailureWrongBaseLocation() throws Exception {
    ...
    IcebergS3SignParams icebergSigner =
        newBuilder()
            .writeLocations(List.of("s3://wrong-bucket/warehouse/ns/table1_cafebabe_wrong"))
            .build();
    Uni<IcebergS3SignResponse> response = icebergSigner.verifyAndSign();
  ...
  private ImmutableIcebergS3SignParams.Builder newBuilder() {
    return ImmutableIcebergS3SignParams.builder()
        ...
        .key(key)
        .warehouseLocation(warehouseLocation)
        .addWriteLocations(tableLocation)
